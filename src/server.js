...
altersyn.get('/print/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const date=value=>{const normalized=normalizeDate(value);return `${normalized.slice(8,10)}.${normalized.slice(5,7)}.${normalized.slice(0,4)}`;};
    res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><title>Altersyn group ${html(record.group)}</title><style>
      ...
      .page{width:297mm;height:210mm}.half{width:148.5mm;height:210mm;padding:10mm;display:flex;flex-direction:column;gap:7mm;border-right:1px dashed #aaa;overflow:hidden}
      .group{text-align:center;border:4px solid #173f2e;border-radius:7mm;padding:4mm 3mm;background:#e9f3ec}.group small{display:block;font-size:16pt;font-weight:800;letter-spacing:.14em}.group strong{display:block;font-size:58pt;line-height:1;font-weight:900}
      .treatment{flex:1;display:flex;flex-direction:column;justify-content:center;border:4px solid #173f2e;border-radius:7mm;padding:5mm;background:#fff}.treatment.altersyn{background:#dcece2}
      h2{margin:0 0 4mm;text-align:center;font-size:27pt;line-height:1;font-weight:900;letter-spacing:.06em}.dates{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:3mm;text-align:center}.date-label{display:block;font-size:12pt;font-weight:800;text-transform:uppercase}.date-value{display:block;margin-top:1mm;font-size:24pt;line-height:1;font-weight:900}.arrow{font-size:25pt;font-weight:900}
