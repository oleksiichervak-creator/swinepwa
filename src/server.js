...
doneSowInjections.use(createSowReportsRouter(pool));
...
function createTableCrud({table,alias,select,order,fields,validate,label}){
  const router=express.Router();
  router.get('/',requireAuth,async(_req,res,next)=>{try{res.json((await pool.query(`${select} ORDER BY ${order}`)).rows);}catch(error){next(error);}});
  router.post('/',requireAuth,requireAdmin,async(req,res,next)=>{try{const input=validate(req.body),values=fields.map(field=>input[field]);const marks=fields.map((_,index)=>`$${index+1}`).join(',');const inserted=await pool.query(`INSERT INTO ${table}(${fields.join(',')}) VALUES(${marks}) RETURNING id`,values);res.status(201).json((await pool.query(`${select} WHERE ${alias}.id=$1`,[inserted.rows[0].id])).rows[0]);}catch(error){handleDbError(error,res,next);}});
  router.get('/:id',requireAuth,async(req,res,next)=>{try{const result=await pool.query(`${select} WHERE ${alias}.id=$1`,[req.params.id]);if(!result.rows[0])return res.status(404).json({error:`${label} not found`});res.json(result.rows[0]);}catch(error){next(error);}});
  router.patch('/:id',requireAuth,requireAdmin,async(req,res,next)=>{try{if(!fields.some(field=>Object.hasOwn(req.body,field)))return res.status(400).json({error:'No fields to update'});const current=await pool.query(`SELECT ${fields.join(',')} FROM ${table} WHERE id=$1`,[req.params.id]);if(!current.rows[0])return res.status(404).json({error:`${label} not found`});const input=validate({...current.rows[0],...req.body}),values=fields.map(field=>input[field]),assignments=fields.map((field,index)=>`${field}=$${index+1}`).join(',');values.push(req.params.id);await pool.query(`UPDATE ${table} SET ${assignments},updated_at=NOW() WHERE id=$${values.length}`,values);res.json((await pool.query(`${select} WHERE ${alias}.id=$1`,[req.params.id])).rows[0]);}catch(error){handleDbError(error,res,next);}});
  router.delete('/:id',requireAuth,requireAdmin,async(req,res,next)=>{try{const result=await pool.query(`DELETE FROM ${table} WHERE id=$1 RETURNING id`,[req.params.id]);if(!result.rows[0])return res.status(404).json({error:`${label} not found`});res.status(204).end();}catch(error){handleDbError(error,res,next);}});
  return router;
}

const vaccines=createTableCrud({table:'vaccines',alias:'v',label:'Vaccine',fields:['name','protects_against','dose_ml'],validate:validateVaccine,
  select:'SELECT v.id,v.name,v.protects_against,v.dose_ml::float8 AS dose_ml,v.created_at,v.updated_at FROM vaccines v',order:'lower(v.name),v.id'});
...
function requiredId(value,label){const id=Number(value);if(!Number.isInteger(id)||id<1)throw Object.assign(new Error(`${label} is required`),{status:400});return id;}
function wholeNumber(value,label,min,max=Number.MAX_SAFE_INTEGER){const number=Number(value);if(!Number.isInteger(number)||number<min||number>max)throw Object.assign(new Error(`${label} must be a whole number from ${min} to ${max}`),{status:400});return number;}
function nonNegativeNumber(value,label){const number=Number(value);if(!Number.isFinite(number)||number<0)throw Object.assign(new Error(`${label} must be a non-negative number`),{status:400});return number;}
function vaccineRecipient(value){const recipient=String(value||'').trim().toLowerCase();if(!['all','polte'].includes(recipient))throw Object.assign(new Error('Recipient must be all or polte'),{status:400});return recipient;}
function validateVaccine(body){const name=String(body.name||'').trim(),protectsAgainst=String(body.protects_against||'').trim();if(!name||name.length>200)throw Object.assign(new Error('Name is required and must not exceed 200 characters'),{status:400});if(!protectsAgainst||protectsAgainst.length>500)throw Object.assign(new Error('Protects against is required and must not exceed 500 characters'),{status:400});return{name,protects_against:protectsAgainst,dose_ml:nonNegativeNumber(body.dose_ml,'Dose')};}
function validateVaccinationSchedule(body){return{vaccine_id:requiredId(body.vaccine_id,'Vaccine'),week_after_insemination:wholeNumber(body.week_after_insemination,'Week after insemination',0,53),recipient:vaccineRecipient(body.recipient)};}
function validatePlannedVaccine(body){const groupNumber=String(body.group_number||'').trim();if(!groupNumber||groupNumber.length>150)throw Object.assign(new Error('Group number is required and must not exceed 150 characters'),{status:400});return{vaccine_id:requiredId(body.vaccine_id,'Vaccine'),week_number:wholeNumber(body.week_number,'Week number',1,53),group_number:groupNumber,recipient:vaccineRecipient(body.recipient)};}
function validateDoneVaccine(body){return{...validatePlannedVaccine(body),pig_count:wholeNumber(body.pig_count,'Pig count',0),vaccine_used_ml:nonNegativeNumber(body.vaccine_used_ml,'Vaccine used'),vaccination_date:normalizeDate(body.vaccination_date,'Vaccination date'),given_by_user_id:requiredId(body.given_by_user_id,'Given by')};}
