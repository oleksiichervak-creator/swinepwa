...
export async function initializeDatabase() {
  ...
  await pool.query(`
    ...
    FROM departments d
    JOIN rooms r ON r.department_id = d.id
    CROSS JOIN generate_series(901, 984) AS pen_number
    WHERE d.name = 'Lobestald' AND r.name = 'L2'
    ON CONFLICT (room_id, name) DO NOTHING
  `);
  ...
  await pool.query(`ALTER TABLE done_sow_injections ADD COLUMN IF NOT EXISTS weight_kg INTEGER CHECK (weight_kg > 0)`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS altersyn (
      id BIGSERIAL PRIMARY KEY,
      "group" VARCHAR(150) NOT NULL,
      ventil VARCHAR(150) NOT NULL,
      amount INTEGER NOT NULL CHECK (amount >= 0),
      juice_start_date DATE NOT NULL,
      juice_stop_date DATE NOT NULL,
      altersyn_start_date DATE NOT NULL,
      altersyn_stop_date DATE NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      CHECK (juice_stop_date >= juice_start_date),
      CHECK (altersyn_stop_date >= altersyn_start_date)
    )
  `);
