...
function renderVaccinationCycle(items){
  ...
  $('#vaccination-cycle').innerHTML=configs.map(config=>{
    ...
    const track=Array.from({length:17},(_,week)=>{const count=(byWeek.get(week)||[]).length,label=week===0?'Insemination':week===16?'Farrowing':`W${week}`;return `<div class="gestation-week ${count?'loaded':''} ${count>1?'heavy':''}"><span class="week-dot">${count||week}</span><span>${label}</span></div>`;}).join('');
    ...
    return `<article class="cycle-card"><div class="cycle-heading"><div><h2>${config.title}</h2><p>${config.subtitle} · 16-week lifecycle</p></div></div><div class="cycle-journey"><figure><img src="/assets/vaccination/sow-walking.png" alt="Sow walking toward farrowing"><figcaption>Insemination</figcaption></figure><span class="journey-arrow" aria-hidden="true">→</span><figure><img src="/assets/vaccination/sow-piglets.png" alt="Sow with piglets at farrowing"><figcaption>Farrowing</figcaption></figure></div><div class="cycle-scroll"><div class="gestation-track">${track}</div></div>${events?`<div class="cycle-events">${events}</div>`:'<p class="cycle-empty">No vaccines scheduled for this lifecycle.</p>'}${outside?`<p class="cycle-outside">${outside} schedule${outside===1?' is':'s are'} outside the 16-week lifecycle and cannot be shown.</p>`:''}</article>`;
  }).join('');
}
async function loadPlannedVaccines(){const items=await api('/planned-vaccines/');$('#planned-vaccines-count').textContent=items.length;$('#planned-vaccines').innerHTML=items.length?items.map(x=>`<tr><td>${x.id}</td><td>${escapeHtml(x.vaccine_name)}</td><td>${x.week_number}</td><td>${escapeHtml(x.group_number)}</td><td><span class="role">${escapeHtml(x.recipient)}</span></td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary planned-vaccine-edit" data-id="${x.id}">Edit</button><button class="danger planned-vaccine-delete" data-id="${x.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="6" class="empty-state">No planned vaccines yet.</td></tr>';document.querySelectorAll('.planned-vaccine-edit').forEach(b=>b.onclick=()=>openPlannedVaccine(items.find(x=>String(x.id)===b.dataset.id)));document.querySelectorAll('.planned-vaccine-delete').forEach(b=>b.onclick=()=>deletePlannedVaccine(b.dataset.id));}
