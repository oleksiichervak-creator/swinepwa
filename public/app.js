...
async function showAdmin() {
  try {
    ...
    $('#add-vaccine').hidden = me.role !== 'admin';
    $('#add-vaccination-schedule').hidden = me.role !== 'admin';
...
async function loadDoneAltresyn(){const items=await api('/done-altersyn/');$('#done-altersyn-count').textContent=items.length;$('#done-altersyn').innerHTML=items.length?items.map(item=>`<tr><td>${item.id}</td><td>${escapeHtml(item.group)}</td><td>${escapeHtml(item.ventil||'—')}</td><td>${item.amount}</td><td>${item.extra_doses}</td><td><strong>${item.total_altersyn_ml} ml</strong></td><td>${formatDisplayDate(item.done_date)}</td><td>${escapeHtml(item.given_by_username)}</td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary done-altersyn-edit" data-id="${item.id}">Edit</button><button class="danger done-altersyn-delete" data-id="${item.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="9" class="empty-state">No completed Altresyn records yet.</td></tr>';document.querySelectorAll('.done-altersyn-edit').forEach(button=>button.onclick=()=>openDoneAltresyn(items.find(item=>String(item.id)===button.dataset.id)));document.querySelectorAll('.done-altersyn-delete').forEach(button=>button.onclick=()=>deleteDoneAltresyn(button.dataset.id));}
...
async function loadPigNews(){const list=$('#pig-news-list'),button=$('#refresh-pig-news');button.disabled=true;list.innerHTML='<div class="news-loading">Loading the latest news…</div>';try{const items=await api('/injection-pwa/news');$('#pig-news-count').textContent=items.length;list.innerHTML=items.map(item=>`<a class="admin-news-card" href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer"><div class="admin-news-meta"><span>${escapeHtml(item.region)} · ${escapeHtml(item.source)}</span><time datetime="${escapeHtml(item.published_at)}">${new Date(item.published_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</time></div><h2>${escapeHtml(item.title)}</h2><span>Read full story →</span></a>`).join('');pigNewsLoaded=true;}catch(error){list.innerHTML=`<div class="news-loading error">${escapeHtml(error.message)}</div>`;}finally{button.disabled=false;}}
async function loadVaccines(){const items=await api('/vaccines/');$('#vaccines-count').textContent=items.length;$('#vaccines').innerHTML=items.length?items.map(x=>`<tr><td>${x.id}</td><td>${escapeHtml(x.name)}</td><td>${escapeHtml(x.protects_against)}</td><td>${x.dose_ml}</td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary vaccine-edit" data-id="${x.id}">Edit</button><button class="danger vaccine-delete" data-id="${x.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="5" class="empty-state">No vaccines yet.</td></tr>';document.querySelectorAll('.vaccine-edit').forEach(b=>b.onclick=()=>openVaccine(items.find(x=>String(x.id)===b.dataset.id)));document.querySelectorAll('.vaccine-delete').forEach(b=>b.onclick=()=>deleteVaccine(b.dataset.id));}
async function loadVaccinationSchedules(){const items=await api('/vaccination-schedules/');$('#vaccination-schedules-count').textContent=items.length;renderVaccinationCycle(items);$('#vaccination-schedules').innerHTML=items.length?items.map(x=>`<tr><td>${x.id}</td><td>${escapeHtml(x.vaccine_name)}</td><td>${x.week_after_insemination}</td><td><span class="role">${escapeHtml(x.recipient)}</span></td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary vaccination-schedule-edit" data-id="${x.id}">Edit</button><button class="danger vaccination-schedule-delete" data-id="${x.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="5" class="empty-state">No vaccination schedules yet.</td></tr>';document.querySelectorAll('.vaccination-schedule-edit').forEach(b=>b.onclick=()=>openVaccinationSchedule(items.find(x=>String(x.id)===b.dataset.id)));document.querySelectorAll('.vaccination-schedule-delete').forEach(b=>b.onclick=()=>deleteVaccinationSchedule(b.dataset.id));}
function renderVaccinationCycle(items){
  const configs=[{title:'Polte',subtitle:'Recipient: all + polte',accept:item=>item.recipient==='all'||item.recipient==='polte'},{title:'Sow',subtitle:'Recipient: all',accept:item=>item.recipient==='all'}];
  $('#vaccination-cycle').innerHTML=configs.map(config=>{
    const applicable=items.filter(config.accept),inside=applicable.filter(item=>Number(item.week_after_insemination)>=0&&Number(item.week_after_insemination)<=16);
    const byWeek=new Map();for(const item of inside){const week=Number(item.week_after_insemination),list=byWeek.get(week)||[];list.push(item);byWeek.set(week,list);}
    const track=Array.from({length:17},(_,week)=>{const count=(byWeek.get(week)||[]).length,label=week===0?'INS':week===16?'FAR':`W${week}`,loadClass=count?`load-${Math.min(4,count)}`:'';return `<div class="gestation-week ${loadClass}"><span class="week-dot">${count||'—'}</span><span>${label}</span></div>`;}).join('');
    const events=[...byWeek].sort((a,b)=>a[0]-b[0]).map(([week,vaccines])=>{const weeksBeforeFarrowing=17-week,timing=week<=8?`${week} week${week===1?'':'s'} after insemination`:`${weeksBeforeFarrowing} week${weeksBeforeFarrowing===1?'':'s'} before farrowing`;return `<div class="cycle-event"><strong>${escapeHtml(timing)}</strong><span>${vaccines.map(item=>escapeHtml(item.vaccine_name)).join(' · ')}</span><span class="vaccine-load" title="Vaccines this week">${vaccines.length}</span></div>`;}).join('');
    const outside=applicable.filter(item=>Number(item.week_after_insemination)>16).length;
    return `<article class="cycle-card"><div class="cycle-heading"><div><h2>${config.title}</h2><p>${config.subtitle} · 16-week lifecycle</p></div></div><div class="cycle-visual"><figure><img src="/assets/vaccination/sow-walking.png" alt="Sow walking toward farrowing"></figure><div class="cycle-scroll"><div class="gestation-track">${track}</div></div><figure><img src="/assets/vaccination/sow-piglets.png" alt="Sow with piglets at farrowing"></figure></div>${events?`<div class="cycle-events">${events}</div>`:'<p class="cycle-empty">No vaccines scheduled for this lifecycle.</p>'}${outside?`<p class="cycle-outside">${outside} schedule${outside===1?' is':'s are'} outside the 16-week lifecycle and cannot be shown.</p>`:''}</article>`;
  }).join('');
}
...
$('#refresh-pig-news').onclick=()=>{pigNewsLoaded=false;loadPigNews();};
...
async function vaccineOptions(select,selected){const items=await api('/vaccines/');if(!items.length)throw new Error('Create a vaccine first.');select.innerHTML=items.map(x=>`<option value="${x.id}">${escapeHtml(x.name)}</option>`).join('');select.value=selected||items[0].id;}
function openVaccine(item={}){const f=$('#vaccine-form');f.reset();f.elements.id.value=item.id||'';f.name.value=item.name||'';f.protects_against.value=item.protects_against||'';f.dose_ml.value=item.dose_ml??'';$('#vaccine-dialog-title').textContent=item.id?'Edit vaccine':'New vaccine';$('#vaccine-error').textContent='';$('#vaccine-dialog').showModal();}
async function deleteVaccine(id){if(confirm('Delete this vaccine?')){try{await api(`/vaccines/${id}`,{method:'DELETE'});await Promise.all([loadVaccines(),loadVaccinationSchedules(),loadPlannedVaccines(),loadDoneVaccines()]);}catch(error){alert(error.message);}}}
$('#add-vaccine').onclick=()=>openVaccine();$('#vaccine-cancel').onclick=()=>$('#vaccine-dialog').close();$('#vaccine-form').onsubmit=async event=>{event.preventDefault();const f=event.currentTarget,id=f.elements.id.value,data={name:f.name.value,protects_against:f.protects_against.value,dose_ml:Number(f.dose_ml.value)};$('#vaccine-error').textContent='';try{await api(id?`/vaccines/${id}`:'/vaccines/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#vaccine-dialog').close();await Promise.all([loadVaccines(),loadVaccinationSchedules(),loadPlannedVaccines(),loadDoneVaccines()]);}catch(error){$('#vaccine-error').textContent=error.message;}};
async function openVaccinationSchedule(item={}){const f=$('#vaccination-schedule-form');f.reset();f.elements.id.value=item.id||'';try{await vaccineOptions(f.vaccine_id,item.vaccine_id);f.week_after_insemination.value=item.week_after_insemination??'';f.recipient.value=item.recipient||'all';$('#vaccination-schedule-dialog-title').textContent=item.id?'Edit vaccination schedule':'New vaccination schedule';$('#vaccination-schedule-error').textContent='';$('#vaccination-schedule-dialog').showModal();}catch(error){alert(error.message);}}
async function deleteVaccinationSchedule(id){if(confirm('Delete this vaccination schedule?')){try{await api(`/vaccination-schedules/${id}`,{method:'DELETE'});await loadVaccinationSchedules();}catch(error){alert(error.message);}}}
$('#add-vaccination-schedule').onclick=()=>openVaccinationSchedule();$('#vaccination-schedule-cancel').onclick=()=>$('#vaccination-schedule-dialog').close();$('#vaccination-schedule-form').onsubmit=async event=>{event.preventDefault();const f=event.currentTarget,id=f.elements.id.value,data={vaccine_id:Number(f.vaccine_id.value),week_after_insemination:Number(f.week_after_insemination.value),recipient:f.recipient.value};$('#vaccination-schedule-error').textContent='';try{await api(id?`/vaccination-schedules/${id}`:'/vaccination-schedules/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#vaccination-schedule-dialog').close();await loadVaccinationSchedules();}catch(error){$('#vaccination-schedule-error').textContent=error.message;}};
async function openPlannedVaccine(item={}){const f=$('#planned-vaccine-form');f.reset();f.elements.id.value=item.id||'';try{await vaccineOptions(f.vaccine_id,item.vaccine_id);f.week_number.value=item.week_number??'';f.group_number.value=item.group_number||'';f.recipient.value=item.recipient||'all';$('#planned-vaccine-dialog-title').textContent=item.id?'Edit planned vaccine':'New planned vaccine';$('#planned-vaccine-error').textContent='';$('#planned-vaccine-dialog').showModal();}catch(error){alert(error.message);}}
async function deletePlannedVaccine(id){if(confirm('Delete this planned vaccine?')){try{await api(`/planned-vaccines/${id}`,{method:'DELETE'});await loadPlannedVaccines();}catch(error){alert(error.message);}}}
$('#add-planned-vaccine').onclick=()=>openPlannedVaccine();$('#planned-vaccine-cancel').onclick=()=>$('#planned-vaccine-dialog').close();$('#planned-vaccine-form').onsubmit=async event=>{event.preventDefault();const f=event.currentTarget,id=f.elements.id.value,data={vaccine_id:Number(f.vaccine_id.value),week_number:Number(f.week_number.value),group_number:f.group_number.value,recipient:f.recipient.value};$('#planned-vaccine-error').textContent='';try{await api(id?`/planned-vaccines/${id}`:'/planned-vaccines/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#planned-vaccine-dialog').close();await loadPlannedVaccines();}catch(error){$('#planned-vaccine-error').textContent=error.message;}};
$('#add-vaccine-group').onclick=()=>{const form=$('#vaccine-group-form');form.reset();form.insemination_year.value=new Date().getFullYear();form.polte_count.value=0;$('#vaccine-group-error').textContent='';$('#vaccine-group-dialog').showModal();};
$('#vaccine-group-cancel').onclick=()=>$('#vaccine-group-dialog').close();
$('#vaccine-group-form').onsubmit=async event=>{event.preventDefault();const form=event.currentTarget,button=event.submitter,data={insemination_year:Number(form.insemination_year.value),group_number:Number(form.group_number.value),total_count:Number(form.total_count.value),polte_count:Number(form.polte_count.value)};button.disabled=true;$('#vaccine-group-error').textContent='';try{const result=await api('/planned-vaccines/make-plan',{method:'POST',body:JSON.stringify(data)});$('#vaccine-group-dialog').close();await loadPlannedVaccines();alert(`${result.plans_created} vaccination plans created for group ${result.group_number}.`);}catch(error){$('#vaccine-group-error').textContent=error.message;}finally{button.disabled=false;}};
$('#print-vaccine-group').onclick=async()=>{const form=$('#vaccine-group-print-form'),select=form.group;$('#vaccine-group-print-error').textContent='';try{const items=await api('/planned-vaccines/'),groups=[...new Map(items.filter(item=>item.insemination_year).map(item=>[`${item.insemination_year}:${item.group_number}`,item])).values()].sort((a,b)=>Number(b.insemination_year)-Number(a.insemination_year)||Number(a.group_number)-Number(b.group_number));if(!groups.length)return alert('Create a vaccination group plan before printing.');select.innerHTML=groups.map(item=>`<option value="${item.insemination_year}:${escapeHtml(item.group_number)}">Group ${escapeHtml(item.group_number)} · ${item.insemination_year}</option>`).join('');$('#vaccine-group-print-dialog').showModal();}catch(error){alert(error.message);}};
$('#vaccine-group-print-cancel').onclick=()=>$('#vaccine-group-print-dialog').close();
$('#vaccine-group-print-form').onsubmit=async event=>{event.preventDefault();const [year,group]=event.currentTarget.group.value.split(':'),popup=window.open('','_blank');if(!popup){$('#vaccine-group-print-error').textContent='Allow pop-ups to open the print page.';return;}popup.document.write('<p style="font:20px Arial;padding:30px">Preparing vaccination plan…</p>');try{const response=await fetch(`/api/planned-vaccines/group-print/${encodeURIComponent(year)}/${encodeURIComponent(group)}`,{headers:{Authorization:`Bearer ${token}`}});if(!response.ok){const error=await response.json().catch(()=>({}));throw new Error(error.error||'Unable to prepare print page');}const url=URL.createObjectURL(await response.blob());popup.location.href=url;$('#vaccine-group-print-dialog').close();setTimeout(()=>URL.revokeObjectURL(url),60000);}catch(error){popup.close();$('#vaccine-group-print-error').textContent=error.message;}};
async function openDoneVaccine(item={}){const f=$('#done-vaccine-form');f.reset();f.elements.id.value=item.id||'';try{const users=await api('/users');await vaccineOptions(f.vaccine_id,item.vaccine_id);f.given_by_user_id.innerHTML=users.map(x=>`<option value="${x.id}">${escapeHtml(x.username)}</option>`).join('');f.week_number.value=item.week_number??'';f.group_number.value=item.group_number||'';f.recipient.value=item.recipient||'all';f.pig_count.value=item.pig_count??'';f.vaccine_used_ml.value=item.vaccine_used_ml??'';f.vaccination_date.value=item.vaccination_date||new Date().toISOString().slice(0,10);f.given_by_user_id.value=item.given_by_user_id||me.id;$('#done-vaccine-dialog-title').textContent=item.id?'Edit done vaccine':'New done vaccine';$('#done-vaccine-error').textContent='';$('#done-vaccine-dialog').showModal();}catch(error){alert(error.message);}}
async function deleteDoneVaccine(id){if(confirm('Delete this completed vaccination?')){try{await api(`/done-vaccines/${id}`,{method:'DELETE'});await loadDoneVaccines();}catch(error){alert(error.message);}}}
$('#add-done-vaccine').onclick=()=>openDoneVaccine();$('#done-vaccine-cancel').onclick=()=>$('#done-vaccine-dialog').close();$('#done-vaccine-form').onsubmit=async event=>{event.preventDefault();const f=event.currentTarget,id=f.elements.id.value,data={vaccine_id:Number(f.vaccine_id.value),week_number:Number(f.week_number.value),group_number:f.group_number.value,recipient:f.recipient.value,pig_count:Number(f.pig_count.value),vaccine_used_ml:Number(f.vaccine_used_ml.value),vaccination_date:f.vaccination_date.value,given_by_user_id:Number(f.given_by_user_id.value)};$('#done-vaccine-error').textContent='';try{await api(id?`/done-vaccines/${id}`:'/done-vaccines/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#done-vaccine-dialog').close();await loadDoneVaccines();}catch(error){$('#done-vaccine-error').textContent=error.message;}};
