...
function renderVaccinationCycle(items){
  ...
  $('#vaccination-cycle').innerHTML=configs.map(config=>{
    ...
    const track=Array.from({length:17},(_,week)=>{const count=(byWeek.get(week)||[]).length,label=week===0?'Insemination':week===16?'Farrowing':`W${week}`,loadClass=count?`load-${Math.min(4,count)}`:'';return `<div class="gestation-week ${loadClass}"><span class="week-dot">${count||'—'}</span><span>${label}</span></div>`;}).join('');
