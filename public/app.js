...
async function showAdmin() {
  try {
    ...
    await Promise.all([loadUsers(), loadDepartments(), loadRooms(), loadPens(), loadMedicineSow(), loadMedicineSowStorage(), loadSowInjections(), loadDoneSow(), loadVetQuestions(), loadFiles(), loadDailyRemarks(), loadRepairLocations(), loadAltersyn(), loadDoneAltersyn(), loadTodos()]);
...
async function loadAltersyn(){const items=await api('/altersyn/');$('#altersyn-count').textContent=items.length;$('#altersyn').innerHTML=items.length?items.map(x=>`<tr><td>${x.id}</td><td>${escapeHtml(x.group)}</td><td>${escapeHtml(x.ventil)}</td><td>${x.amount}</td><td>${formatDisplayDate(x.juice_start_date)}</td><td>${formatDisplayDate(x.juice_stop_date)}</td><td>${formatDisplayDate(x.altersyn_start_date)}</td><td>${formatDisplayDate(x.altersyn_stop_date)}</td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary altersyn-edit" data-id="${x.id}">Edit</button><button class="danger altersyn-delete" data-id="${x.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="9" class="empty-state">No Altersyn records yet.</td></tr>';document.querySelectorAll('.altersyn-edit').forEach(button=>button.onclick=()=>openAltersyn(items.find(item=>String(item.id)===button.dataset.id)));document.querySelectorAll('.altersyn-delete').forEach(button=>button.onclick=()=>deleteAltersyn(button.dataset.id));}
async function loadDoneAltersyn(){const items=await api('/done-altersyn/');$('#done-altersyn-count').textContent=items.length;$('#done-altersyn').innerHTML=items.length?items.map(item=>`<tr><td>${item.id}</td><td>${escapeHtml(item.group)}</td><td>${item.amount}</td><td>${item.extra_doses}</td><td><strong>${item.total_altersyn_ml} ml</strong></td><td>${formatDisplayDate(item.done_date)}</td><td>${escapeHtml(item.given_by_username)}</td><td><div class="row-actions">${me.role==='admin'?`<button class="secondary done-altersyn-edit" data-id="${item.id}">Edit</button><button class="danger done-altersyn-delete" data-id="${item.id}">Delete</button>`:''}</div></td></tr>`).join(''):'<tr><td colspan="8" class="empty-state">No completed Altersyn records yet.</td></tr>';document.querySelectorAll('.done-altersyn-edit').forEach(button=>button.onclick=()=>openDoneAltersyn(items.find(item=>String(item.id)===button.dataset.id)));document.querySelectorAll('.done-altersyn-delete').forEach(button=>button.onclick=()=>deleteDoneAltersyn(button.dataset.id));}
...
function calculateAltersynDates(year,week){const jan4=new Date(Date.UTC(year,0,4)),jan4Day=jan4.getUTCDay()||7,monday=new Date(jan4);monday.setUTCDate(jan4.getUTCDate()-(jan4Day-1)+(week-1)*7);const weekThursday=new Date(monday);weekThursday.setUTCDate(monday.getUTCDate()+3);if(weekThursday.getUTCFullYear()!==year)return null;const dateAtOffset=offset=>{const date=new Date(monday);date.setUTCDate(monday.getUTCDate()+offset);return date.toISOString().slice(0,10);};return{altersyn_stop_date:dateAtOffset(-4),altersyn_start_date:dateAtOffset(-21),juice_stop_date:dateAtOffset(-22),juice_start_date:dateAtOffset(-25)};}
function updateAltersynDates(){const form=$('#altersyn-form'),week=Number(form.group.value),dates=calculateAltersynDates(altersynScheduleYear,week);$('#altersyn-year-note').textContent=`Calendar year: ${altersynScheduleYear}`;$('#altersyn-error').textContent=dates?'':'This week does not exist in the selected calendar year.';for(const field of ['juice_start_date','juice_stop_date','altersyn_start_date','altersyn_stop_date'])form[field].value=dates?.[field]||'';}
function openAltersyn(item={}){const form=$('#altersyn-form');form.reset();form.id.value=item.id||'';altersynScheduleYear=Number(String(item.altersyn_stop_date||'').slice(0,4))||new Date().getFullYear();for(const field of ['group','ventil','amount'])form[field].value=item[field]??'';$('#altersyn-dialog-title').textContent=item.id?'Edit Altersyn record':'New Altersyn record';$('#altersyn-error').textContent='';updateAltersynDates();$('#altersyn-dialog').showModal();}
async function deleteAltersyn(id){if(confirm('Delete this Altersyn record?')){try{await api(`/altersyn/${id}`,{method:'DELETE'});await loadAltersyn();}catch(error){alert(error.message);}}}
$('#print-altersyn').onclick=async()=>{const form=$('#altersyn-print-form'),select=form.elements.id;$('#altersyn-print-error').textContent='';try{const items=await api('/altersyn/');select.innerHTML=items.sort((a,b)=>Number(a.group)-Number(b.group)).map(item=>`<option value="${item.id}">Group ${escapeHtml(item.group)} · ${escapeHtml(item.ventil)}</option>`).join('');if(!items.length)return alert('Create an Altersyn record before printing.');$('#altersyn-print-dialog').showModal();}catch(error){alert(error.message);}};
...
$('#altersyn-form').group.addEventListener('input',updateAltersynDates);$('#add-altersyn').onclick=()=>openAltersyn();$('#altersyn-cancel').onclick=()=>$('#altersyn-dialog').close();$('#altersyn-form').onsubmit=async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.target)),id=data.id;delete data.id;data.amount=Number(data.amount);data.schedule_year=altersynScheduleYear;$('#altersyn-error').textContent='';try{await api(id?`/altersyn/${id}`:'/altersyn/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#altersyn-dialog').close();await loadAltersyn();}catch(error){$('#altersyn-error').textContent=error.message;}};
function updateDoneAltersynTotal(){const fields=$('#done-altersyn-form').elements,amount=Number(fields.amount.value)||0,extra=Number(fields.extra_doses.value)||0;$('#done-altersyn-total').textContent=`Total Altersyn: ${(amount+extra)*5} ml`;}
async function openDoneAltersyn(item={}){const[groups,users]=await Promise.all([api('/altersyn/'),api('/users')]);if(!groups.length)return alert('Create an Altersyn group before adding a completed record.');const form=$('#done-altersyn-form');form.reset();form.id.value=item.id||'';form.group.innerHTML=[...new Map(groups.map(group=>[String(group.group),group])).values()].sort((a,b)=>Number(a.group)-Number(b.group)).map(group=>`<option value="${escapeHtml(group.group)}">Group ${escapeHtml(group.group)}</option>`).join('');form.given_by_user_id.innerHTML=users.map(user=>`<option value="${user.id}">${escapeHtml(user.username)}</option>`).join('');form.group.value=item.group||form.group.options[0].value;form.amount.value=item.amount??'';form.extra_doses.value=item.extra_doses??0;form.done_date.value=item.done_date||new Date().toISOString().slice(0,10);form.given_by_user_id.value=item.given_by_user_id||me.id;$('#done-altersyn-dialog-title').textContent=item.id?'Edit Done Altersyn record':'New Done Altersyn record';$('#done-altersyn-error').textContent='';updateDoneAltersynTotal();$('#done-altersyn-dialog').showModal();}
async function deleteDoneAltersyn(id){if(confirm('Delete this completed Altersyn record?')){try{await api(`/done-altersyn/${id}`,{method:'DELETE'});await loadDoneAltersyn();}catch(error){alert(error.message);}}}
$('#done-altersyn-form').amount.addEventListener('input',updateDoneAltersynTotal);$('#done-altersyn-form').extra_doses.addEventListener('input',updateDoneAltersynTotal);$('#add-done-altersyn').onclick=()=>openDoneAltersyn();$('#done-altersyn-cancel').onclick=()=>$('#done-altersyn-dialog').close();$('#done-altersyn-form').onsubmit=async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget)),id=data.id;delete data.id;for(const field of ['amount','extra_doses','given_by_user_id'])data[field]=Number(data[field]);$('#done-altersyn-error').textContent='';try{await api(id?`/done-altersyn/${id}`:'/done-altersyn/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#done-altersyn-dialog').close();await loadDoneAltersyn();}catch(error){$('#done-altersyn-error').textContent=error.message;}};
