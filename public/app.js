...
function renderVaccinationCycle(items){
  ...
  $('#vaccination-cycle').innerHTML=configs.map(config=>{
    ...
    const events=[...byWeek].sort((a,b)=>a[0]-b[0]).map(([week,vaccines])=>{const timing=week<=8?`${week} week${week===1?'':'s'} after insemination`:`${16-week} week${16-week===1?'':'s'} before farrowing`;return `<div class="cycle-event"><strong>${escapeHtml(timing)}</strong><span>${vaccines.map(item=>escapeHtml(item.vaccine_name)).join(' · ')}</span><span class="vaccine-load" title="Vaccines this week">${vaccines.length}</span></div>`;}).join('');
