...
function openAltersyn(item={}){const form=$('#altersyn-form');form.reset();form.id.value=item.id||'';for(const field of ['group','ventil','amount','juice_start_date','juice_stop_date','altersyn_start_date','altersyn_stop_date'])form[field].value=item[field]??'';$('#altersyn-dialog-title').textContent=item.id?'Edit Altersyn record':'New Altersyn record';$('#altersyn-error').textContent='';$('#altersyn-dialog').showModal();}
...
$('#add-altersyn').onclick=()=>openAltersyn();$('#altersyn-cancel').onclick=()=>$('#altersyn-dialog').close();$('#altersyn-form').onsubmit=async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.target)),id=data.id;delete data.id;data.amount=Number(data.amount);$('#altersyn-error').textContent='';try{await api(id?`/altersyn/${id}`:'/altersyn/',{method:id?'PATCH':'POST',body:JSON.stringify(data)});$('#altersyn-dialog').close();await loadAltersyn();}catch(error){$('#altersyn-error').textContent=error.message;}};
